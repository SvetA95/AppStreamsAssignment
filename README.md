...
- **UI and API specs run as two Cypress invocations.** `video` is a whole-run setting in Cypress, not per-spec, so recording the API spec (pure `cy.request`, no browser action worth watching back) would mean either video for everything or nothing. `npm run cy:run` / CI instead run `cypress/e2e` and `cypress/api` separately, disabling video only for the latter. Getting both to land in one combined report needs three settings working together: `cypressParallel` and `removeJsonsFolderAfterMerge: false` (`reporterOptions`) stop the reporter wiping the first invocation's results before the second one merges in, and `trashAssetsBeforeRuns: false` stops Cypress's own default behavior from deleting the first invocation's screenshot file out from under that merge — which is what an explicit `npm run clean` is for, run once at the start of `cy:run` instead.
